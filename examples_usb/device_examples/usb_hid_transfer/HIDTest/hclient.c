...
Abstract:
    ...
    This module contains the code for handling HClient's main dialogue box and 
...
//****************************************************************************
// Macro definition to get device block from the main dialogue box procedure
//****************************************************************************
...
)
{
    ...
    //  static linking of HID.DLL to this application on compilation.  However,
    ...
    //   so we'll throw up a message box and then exit the application
    //
    ...
    switch (message)
    {
        case WM_INITDIALOG:
            ...
            for (iIndex = 0; (ULONG) iIndex < numberDevices; iIndex++, pDevice++)
            {
                ...
                //  window can receive notification if this device is unplugged.
                //
            ...
            SendDlgItemMessage(hDlg, IDC_CHECK_OH, BM_SETCHECK, BST_UNCHECKED, 0);
            SendDlgItemMessage(hDlg, IDC_CHECK_IH, BM_SETCHECK, BST_CHECKED, 0);
            SendDlgItemMessage(hDlg, IDC_EDIT_OUT, WM_SETTEXT, 0, (LPARAM)"Enter Output");
            ...
            switch(LOWORD(wParam))
            {
                ...
                case IDC_WRITE:
                    ...
                    if (NULL != pDevice) 
                    {
                        ...
                        //  writeable handle to the device.  In this case, the
                ...
                case IDC_CHECK_LED2:
                    ...
                    // LB_: List box, CB_: Combo box, SS_:Static Styles
                    // BM_: Button
                    {
                    int bst1 = (int)SendDlgItemMessage(hDlg, IDC_CHECK_LED1, BM_GETCHECK, 0, 0);
                    int bst2 = (int)SendDlgItemMessage(hDlg, IDC_CHECK_LED2, BM_GETCHECK, 0, 0);
                    BYTE tmpBuffer[64 * 6];
                    DWORD bytesWritten;
                        ...
                        if (!OpenHidDevice(pDevice->DevicePath,
                            FALSE, TRUE, FALSE, FALSE, &hidDevice) ) {
                            MessageBox(hDlg, "Can not open HID Device!", "Err LED", MB_ICONSTOP);
                            break;
                        }

                        tmpBuffer[0] = 0;
                        tmpBuffer[1] = 0x80;
                        if (bst1) tmpBuffer[1] |= 0x01;
                        if (bst2) tmpBuffer[1] |= 0x02;
                        if (!WriteFile(hidDevice.HidDevice,
                                        tmpBuffer,
                                        hidDevice.Caps.OutputReportByteLength,
                                        &bytesWritten, NULL)) {
                            MessageBox(hDlg, "Failed to write!", "Err LED", MB_ICONSTOP);
                    ...
                    if (NULL != pDevice)
                    {
                        ...
                        if (!OpenHidDevice(pDevice -> DevicePath, 
                            ...
                            if (!WriteFile(hidDevice.HidDevice,
                                        ...
                                        &bytesWritten,
                                        NULL)) {
                                MessageBox(hDlg, "Failed to write!", "Err Write", MB_ICONSTOP);
                            }
                        }
                        CloseHidDevice(&hidDevice);
                    }
                ...
                case IDC_CHECK_MON:
                    {
                        int mon = (int)SendDlgItemMessage(hDlg, IDC_CHECK_MON, BM_GETCHECK, 0, 0);
                        // Start monitor
                        if (mon == BST_CHECKED) {
                            ...
                            if (!OpenHidDevice(pDevice->DevicePath,
                                ...
                                CheckDlgButton(hDlg, IDC_CHECK_MON, BST_UNCHECKED);
                                break;
                            }
                            ...
                            readThread = CreateThread(NULL,
                                                        ...
                                                        &threadID);
                            if (NULL == readThread) {
                                MessageBox(hDlg, "Fail to create read thread", "Err MON", MB_ICONEXCLAMATION);
                                CloseHidDevice(&hidDevice);
                                SendDlgItemMessage(hDlg, IDC_CHECK_MON, BM_SETCHECK, BST_UNCHECKED, 0);
                            }
                        }
                        else {
                ...
                case IDC_BUTTON_RD:
                    ...
                    if (NULL != pDevice)
                    {
                        ...
                        if (!Read(&hidDevice)) {
                            MessageBox(hDlg, "Failed to read!", "Err read", MB_ICONSTOP);
                        }
                        else {
                            // Dump buffer
                            int i;
                            ...
                            for(i = 1; i <= hidDevice.Caps.InputReportByteLength; i ++) {
                                ...
                                *pBuff ++ = HalfBtoHex(hidDevice.InputReportBuffer[i] >> 4);
                                *pBuff ++ = HalfBtoHex(hidDevice.InputReportBuffer[i]);
                                *pBuff ++ = ' ';
                            }
                            *pBuff = 0;
                            ...
                            if (hidDevice.InputReportBuffer[1] & 0x80) {

                                if (hidDevice.InputReportBuffer[1] & 0x01) {
                                    SendDlgItemMessage(hDlg, IDC_CHECK_BTN1, BM_SETCHECK, BST_CHECKED, 0);
                                }
                                else {
                                    SendDlgItemMessage(hDlg, IDC_CHECK_BTN1, BM_SETCHECK, BST_UNCHECKED, 0);
                                }
                                if (hidDevice.InputReportBuffer[1] & 0x02) {
                                    SendDlgItemMessage(hDlg, IDC_CHECK_BTN2, BM_SETCHECK, BST_CHECKED, 0);
                                }
                                else {
                                    SendDlgItemMessage(hDlg, IDC_CHECK_BTN2, BM_SETCHECK, BST_UNCHECKED, 0);
                                }
                                CheckDlgButton(hDlg,
                                               IDC_CHECK_LEFT,
                                               (hidDevice.InputReportBuffer[1] & 0x04) ?
                                                BST_CHECKED : BST_UNCHECKED);
                                CheckDlgButton(hDlg,
                                               IDC_CHECK_UP,
                                               (hidDevice.InputReportBuffer[1] & 0x08) ?
                                                BST_CHECKED : BST_UNCHECKED);
                                CheckDlgButton(hDlg,
                                               IDC_CHECK_DOWN,
                                               (hidDevice.InputReportBuffer[1] & 0x10) ?
                                                BST_CHECKED : BST_UNCHECKED);
                                CheckDlgButton(hDlg,
                                               IDC_CHECK_RIGHT,
                                               (hidDevice.InputReportBuffer[1] & 0x20) ?
                                                BST_CHECKED : BST_UNCHECKED);
                            }
                        }
                        CloseHidDevice(&hidDevice);
                    ...
                    if (NULL != pDevice)
                    {
                        ...
                        if (!OpenHidDevice(pDevice -> DevicePath, 
                            ...
                            if (!HidD_SetOutputReport(hidDevice.HidDevice,
                                                      tmpBuffer,
                                                      hidDevice.Caps.OutputReportByteLength)) {
                                MessageBox(hDlg, "Failed to set report!", "Err SetReport", MB_ICONSTOP);
                            }
                        }
                        CloseHidDevice(&hidDevice);
                    }
                ...
                case IDC_BUTTON_GET:
                    ...
                    if (NULL != pDevice)
                    {
                        ...
                        if (!HidD_GetInputReport(hidDevice.HidDevice,
                            ...
                            MessageBox(hDlg, "Failed to get report!", "Err GetReport", MB_ICONSTOP);
                        }
                        else {
                            // Dump buffer
                            int i;
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam))
            {
                ...
                case IDC_READ_ASYNCH_CONT:
                    ...
                    //  2) Stop a previous asynch read thread
                    //
...
)
{
    ...
    do 
    {
        ...
        {
            //
            // Wait for the completion event to be signalled or a timeout
            //
            
            waitStatus = WaitForSingleObject (completionEvent, READ_THREAD_TIMEOUT );

            //
            // If completionEvent was signalled, then a read just completed
...
#if 1
    do {
        PostMessage(Context->DisplayWindow, WM_COMMAND, IDC_BUTTON_RD, IDC_BUTTON_RD);
        Sleep(100);
    } while ( !Context->TerminateThread && !Context->DoOneRead );
#else
    do 
